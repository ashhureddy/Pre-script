...
def _cell_matches_node(cell, node_name):
    """Does this cell belong to this node? Confirmed real case that plain
    str(cell).startswith(node_name) gets WRONG: node 'FCL09220R' owns cells
    named 'FCL09220_3A_1' — the node name carries a trailing suffix its own
    cells don't, so startswith() silently dropped all 3 of that node's
    cells from every check that filtered this way.

    Matches either direction: the cell name starts with the node name (the
    ordinary case), OR the node name starts with the cell's own prefix (the
    suffix case above). Still rejects a genuinely different node — cell
    'FCL04120_7B_1' vs node 'FCL09220R' fails both directions."""
    if not cell or not node_name:
        return False
    c, n = str(cell).strip().upper(), str(node_name).strip().upper()
    return c.startswith(n) or n.startswith(c.split('_')[0])
...
def check_radio_type(node_id, log_text, ciq_wb, rfds_pages, e_name, g_name, node_logs=None, moved_map=None):
    ...
    for row in _rows(ciq_wb, 'eUtran Parameters'):
        ...
        if not (cell and e_name and _cell_matches_node(cell, e_name)):
            continue
...
def check_rfds_cell_presence(node_id, ciq_wb, rfds_pages):
    ...
    for row in _rows(ciq_wb, 'eUtran Parameters'):
        cell = row.get('EutranCellFDDId')
        if not cell or not _cell_matches_node(cell, e_name):
            continue
...
def check_cells_vs_rfds(node_id, ciq_wb, rfds_pages, e_name, g_name):
    ...
    for row in _rows(ciq_wb, 'eUtran Parameters'):
        cell = row.get('EutranCellFDDId')
        if cell and e_name and _cell_matches_node(cell, e_name):
            ciq_cells.add(cell)
            present = cell in rfds_cells
            label, sector = band_label(cell)
...
def check_ssb_5g(node_id, parsed, log_text, ciq_wb, has_pre_log, node_logs=None, moved_map=None, g_name=None):
    ...
    for row in _rows(ciq_wb, '5G Info'):
        ...
        if not cell:
            continue
        # Node filter — same confirmed bug as check_rf_params_4g's: without
        # it, every node in the run evaluated every 5G cell present in its
        # own log extraction, regardless of which node owns the cell.
        if g_name and not _cell_matches_node(cell, g_name):
            continue
        pre_vals = pre_5g.get(cell)
        if not pre_vals:
            continue
...
def check_arfcn_bw_5g(node_id, parsed, log_text, ciq_wb, has_pre_log, node_logs=None, moved_map=None, g_name=None):
...
def check_rilink_vs_rbb_4g(node_id, log_text, ciq_wb, e_name, node_logs=None, moved_map=None):
    ...
    for row in _rows(ciq_wb, 'eUtran Parameters'):
        cell = row.get('EutranCellFDDId')
        if not (cell and _cell_matches_node(cell, e_name)):
            continue
        pre_ri = rilink.get(cell, 'NA')
        rbb = row.get('RBB type')
        ciq_ri = pe.parse_rbb_link(rbb)
...
def check_rf_params_4g(node_id, log_text, ciq_wb, has_pre_log, retuned_cells=None, node_logs=None, moved_map=None, e_name=None):
    ...
    for row in _rows(ciq_wb, 'eUtran Parameters'):
        ...
        if not cell:
            continue
        # Node filter — confirmed real bug without it: this function had NO
        # node scoping at all, so for every node in the run it evaluated
        # every CIQ cell that appeared in THAT node's Pre log extraction,
        # regardless of which node the cell actually belongs to. On a real
        # site that meant node FCL09220R reporting results for 9 cells
        # owned by FCL04120 (its log captures neighbouring cells), with the
        # same cell evaluated twice under two different node names and
        # mismatch counts inflated accordingly.
        if e_name and not _cell_matches_node(cell, e_name):
            continue
        pre_vals = pre_lte.get(cell)
        if not pre_vals:
            continue
...
def check_rf_params_5g(node_id, parsed, log_text, ciq_wb, has_pre_log, retuned_cells=None, node_logs=None, moved_map=None, g_name=None):
...
def check_losses_vs_antenna_sectors(node_id, ciq_wb, e_name=None, g_name=None):
    ...
    def _cells(sheet):
        ...
        for r in _rows(ciq_wb, sheet):
            cell = r.get('EutranCellFDDId')
            if cell and _cell_matches_node(cell, prefix):
