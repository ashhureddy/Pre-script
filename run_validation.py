...
def run(ciq_path, edp_path, rfds_path, node_logs_text, out_pdf):
    ...
    results = {k: [] for k in (
        ...
        'cells_vs_rfds', 'cell_id_vs_rfds', 'cell_id_vs_rfds_rcn', 'cellid_uniqueness_4g', 'nrcelldu_nrcellcu', 'antenna_type_rfds', 'gnb_identity', 'gnb_du_type', 'enb_identity',
    ...
    for node_id in checked_nodes:
        ...
        g_name = cer.resolve_g_name(ciq_wb, mm_row, e_name, rfds_pages, rfds_bytes, mm_rows) if mm_row else None
        ...
        results['antenna_type_rfds'] += cs.check_antenna_type_vs_rfds(node_id, ciq_wb, rfds_pages, g_name, e_name, rfds_bytes)
        results['gnb_identity'] += cs.check_gnb_identity_consistency(node_id, ciq_wb, g_name, mm_row)
        ...
        results['cell_id_vs_rfds_rcn'] += cs.check_cell_id_vs_rfds_rcn(node_id, ciq_wb, rfds_pages, e_name, g_name)
        ...
        results['ssb_5g'] += cs.check_ssb_5g(node_id, parsed, log_text, ciq_wb, has_pre, node_logs, moved_map)
        ...
        results['eutranfreq_limit'] += cs.check_eutranfreq_limit(node_id, log_text)
        results['maxfreqcheck'] += cs.check_maxfreqcheck(node_id, log_text)
        results['vonr_prelog'] += cs.check_vonr_prelog(node_id, log_text)
        results['vonr_vs_ciq'] += cs.check_vonr_vs_ciq(node_id, log_text, ciq_wb)
        results['radio_port'] += cs.check_radio_port(node_id, log_text, ciq_wb)
